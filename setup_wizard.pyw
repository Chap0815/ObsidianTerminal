...
class SetupWizard(ctk.CTk):
    ...
    def _write_env_file(self):
        lines = [
            ...
            f"API_KEY={self.data['api_key']}",
            f"API_SECRET={self.data['api_secret']}",
        ]

        if EXCHANGES[self.data["exchange"]]["passphrase"]:
            lines.append(f"API_PASSPHRASE={self.data['passphrase']}")

        lines += ["", "# ── Proxy ──────────────────────────────────────────"]
        lines.append(f"USE_PROXY={'true' if self.data['use_proxy'] else 'false'}")
        if self.data["use_proxy"]:
            lines.append(f"PROXY_HOST={self.data['proxy_host']}")
            lines.append(f"PROXY_PORT={self.data['proxy_port']}")

        if self.data["use_telegram"]:
            lines += ["", "# ── Telegram ────────────────────────────────────────"]
            lines.append(f"TELEGRAM_TOKEN={self.data['telegram_token']}")
            lines.append(f"TELEGRAM_CHAT_ID={self.data['telegram_chat_id']}")

        if self.data["use_cryptopanic"]:
            lines += ["", "# ── CryptoPanic API ─────────────────────────────────"]
            lines.append(f"CRYPTOPANIC_TOKEN={self.data['cryptopanic_token']}")

        if self.data.get("cmc_api_key"):
            lines += ["", "# ── CoinMarketCap API (Fear & Greed) ────────────────"]
            lines.append(f"CMC_API_KEY={self.data['cmc_api_key']}")
        ...
        lines += [
            ...
            f"BOT_TIMEZONE={self.data.get('timezone', 'UTC')}",
        ]
