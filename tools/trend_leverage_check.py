...
UNIVERSE_30 = ["BTC", "ETH", "BNB", "XRP", "SOL", "ADA", "AVAX", "LINK", "DOT",
               "LTC", "DOGE", "TRX", "MATIC", "ATOM", "UNI", "ETC", "XLM", "BCH",
               "FIL", "NEAR", "ICP", "ALGO", "HBAR", "VET", "AAVE", "EGLD",
               "SAND", "MANA", "THETA", "AXS"]
...
def ensemble(c: np.ndarray, h: np.ndarray, l: np.ndarray) -> np.ndarray:
...
def backtest_lev(closes: np.ndarray, sig: np.ndarray, lev: float,
                 cost: float = COST, mm: float = MAINT_MARGIN) -> dict:
    ...
    for i in range(1, n):
        ...
        if target == 1 and pos == 0:
            eq *= (1 - cost); entry = eq; pos = 1
        elif target == 0 and pos == 1:
            eq *= (1 - cost); trades += 1; entry = None; pos = 0
        if pos == 1:
            bars_in += 1
            ...
            if factor <= mm or (entry is not None and eq * factor <= entry * mm):
                ...
                liqs += 1; trades += 1; entry = None; pos = 0
            else:
                eq *= factor
        if eq > peak:
            peak = eq
        ...
        if dd > maxdd:
            maxdd = dd
        curve[i] = eq
    return {"ret": (eq - 1) * 100.0, "maxdd": maxdd * 100.0,
            "trades": trades, "liqs": liqs,
            "pct_in": 100.0 * bars_in / max(1, n - 1), "curve": curve}
...
def _get_symbols_for_size(ex, n: int, days: int, fallback_symbols: list[str]) -> list[str]:
    """Return top-N USDT-pair symbols from exchange by $-volume, listing-age filtered."""
    syms = get_top_volume_coins(ex, n, days)
    if syms:
        return syms
    print(f"   [warn] volume fetch failed for N={n}, falling back to hardcoded list (first {n})")
...
def _run_sweep(ex, days: int, custom_coins: list[str]) -> None:
    ...
    print(f"  {days}d | leverage: {', '.join(f'{l:g}x' for l in LEVERAGES)} | "
    ...
    if custom_coins:
        ...
        for n in UNIVERSE_SIZES:
            ...
            print(f"   {len(data)}/{len(syms)} coins qualified with >={min_bars} daily bars")
    ...
    for n in sweep_sizes:
        ...
        series = [(c, h, l, ensemble(c, h, l)) for c, h, l in data.values()]
        K = min(len(c) for c, h, l, _ in series)
        series = [(c[-K:], s[-K:]) for c, h, l, s in series]
        bh = float(np.mean([(c[-1] / c[0] - 1) * 100 for c, _ in series]))

        print(f"  {label.upper()}  ({len(series)} coins, B&H avg {bh:+.1f}%)  ")
        print(f"  {'Lev':>4}  {'PortRet%':>9}  {'PortDD%':>8}  {'Liqs':>6}  "
              f"{'Trd/wk':>8}  {'Deploy?':>8}")
        print("  " + "-" * 58)
        ...
        for lev in LEVERAGES:
            ...
            print(f"  {lev:>3g}x  {port_ret:>+8.1f}%  {port_dd:>7.1f}%  {tot_liq:>6}  "
    ...
    if best_cell:
        ...
        print(f"  CONCLUSION: Best risk-adjusted deployable cell = {label}, {lev:g}x leverage "
              f"({ret:+.1f}% return, {dd:.1f}% PortDD, 0 liquidations).")
    else:
        print("  CONCLUSION: No cell meets the deployment gate (PortDD<40% AND 0 liquidations) "
