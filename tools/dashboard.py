                ...
                for _, row in df.iterrows():
                    existing.add(
                        (
                            str(
                                row.get("base_bot")
                                or _base_bot_name(row.get("bot_name", ""))
                            ).upper(),
                            str(
                                row.get("mode")
                                or _mode_for_current_row(row.get("bot_name", ""))
                            ).upper(),
...
with st.sidebar:
    st.markdown(
        '<div class="section-title" style="margin-top:0;">REFRESH</div>',
        unsafe_allow_html=True,
    )
    ...
    period_choice = st.selectbox(
        "Time period",
        ["All time", "Last 24h", "Last 7 days", "Last 30 days", "Last 90 days"],
        index=0,
    )
    bot_filter = st.multiselect("Bots", ALL_BOTS, default=ALL_BOTS)
...
with tab_overview:
    ...
    if not open_fut_stale_all.empty:
        stale_bots = ", ".join(
            sorted(
                {
                    str(v)
                    for v in open_fut_stale_all.get("bot_name", [])
                    if str(v).strip()
                }
            )
        )
    ...
    for idx, bot in enumerate(ALL_BOTS):
        ...
        def _bucket_has_money(mode: str) -> bool:  # noqa: B023 - called in-loop only
        ...
        bot_trades = (
            mode_trades[mode_trades[bot_trade_col] == bot]
    ...
    if live_trades.empty:
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                gridcolor="rgba(255,255,255,0.04)", title="USDT", showline=False
            ),
        ...
        with col_right:
            ...
            st.markdown(
                _kpi_card("MAX DRAWDOWN", -max_dd, "worst peak-to-trough", "#ef4444"),
                unsafe_allow_html=True,
            )
            st.markdown(
                _kpi_card("CURRENT DD", -current_dd, "from latest peak", "#f59e0b"),
                unsafe_allow_html=True,
            )
...
with tab_trades:
    ...
    if trades.empty:
        ...
        with fcols[1]:
            outcome_f = st.selectbox(
                "Outcome", ["All", "Wins only", "Losses only"], index=0
            )
        with fcols[2]:
        ...
        with fcols[4]:
            type_f = st.selectbox("Type", ["All", "Spot only", "Futures only"], index=0)
        ...
        with m2:
            st.markdown(
                _kpi_card(
                    "AVG TRADE", sub_metrics.get("avg_trade", 0), "per trade", "#b07ae0"
                ),
                unsafe_allow_html=True,
            )
        with m3:
            st.markdown(
                _kpi_card(
                    "BEST", sub_metrics.get("best", 0), "single trade", "#22c55e"
                ),
                unsafe_allow_html=True,
            )
        with m4:
            st.markdown(
                _kpi_card(
                    "WORST", sub_metrics.get("worst", 0), "single trade", "#ef4444"
                ),
                unsafe_allow_html=True,
            )
        ...
        # fmt: off - source-level regression tests protect this column contract.
        ...
        display_fmt["Entry"] = display_fmt["Entry"].apply(
            lambda v: f"{v:.6f}" if pd.notna(v) else ""
        )
        display_fmt["Exit"] = display_fmt["Exit"].apply(
            lambda v: f"{v:.6f}" if pd.notna(v) else ""
        )
        display_fmt["P&L %"] = display_fmt["P&L %"].apply(
            lambda v: f"{v:+.2f}%" if pd.notna(v) else ""
        )
        display_fmt["P&L USDT"] = display_fmt["P&L USDT"].apply(
            lambda v: f"{v:+.2f}" if pd.notna(v) else ""
        )
        display_fmt["Size"] = display_fmt["Size"].apply(
            lambda v: f"{v:.2f}" if pd.notna(v) else ""
        )
...
with tab_positions:
    ...
    open_spot_stale = [
        row for row in open_spot if row.get("state_is_stale") or row.get("state_error")
    ]
    open_spot_active = [
        row
        for row in open_spot
    ...
    # fmt: off - keep the audited open-symbol-only contract plainly visible.
    ...
    def _render_spot_cards(_rows):
        ...
        for idx, t in enumerate(_rows):
            with cols[idx % len(cols)]:
                ...
                pnl = (
                    spot_unrealized_pnl(buy, live, amount) if not price_missing else 0.0
                )
                ...
                pnl_text = (
                    "price unavailable" if price_missing else f"{sign}{pnl:.2f} USDT"
                )
                partial_badge = (
    ...
    for _label, _bot in (
        ...
        _fut_bot_col = (
            "base_bot" if "base_bot" in open_fut_active.columns else "bot_name"
        )
        # fmt: off - keep bot scoping explicit for the source regression gate.
        ...
        # fmt: on
        _live_sub = _sub[_sub["mode"] == "LIVE"] if "mode" in _sub.columns else _sub
        _sim_sub = (
            _sub[_sub["mode"] == "SIM"] if "mode" in _sub.columns else _sub.iloc[0:0]
        )
        _section(f"Open {_label} LIVE Positions")
...
with tab_performance:
    if trades.empty:
        ...
        st.info(
            "No LIVE trades in the selected period. SIM/LEGACY trades are excluded from money performance."
        )
    else:
        ...
        with rc4:
            ...
            st.markdown(
                _kpi_card("EXPECTANCY", exp, "expected $ per trade", "#6a5cc0"),
                unsafe_allow_html=True,
            )
        ...
        with rc5:
            st.markdown(
                _kpi_card("BEST TRADE", m.get("best", 0), "single profit", "#22c55e"),
                unsafe_allow_html=True,
            )
        with rc6:
            st.markdown(
                _kpi_card("WORST TRADE", m.get("worst", 0), "single loss", "#ef4444"),
                unsafe_allow_html=True,
            )
        with rc7:
            st.markdown(
        ...
        with hm_left:
            ...
            hour_stats = all_hours.merge(
                hour_stats, on="hour_of_day", how="left"
            ).fillna(0)

            fig_h = go.Figure()
            colors = [
                "#22c55e" if v > 0 else "#ef4444" if v < 0 else "#475569"
        ...
        with hm_right:
            ...
            day_stats = all_days.merge(day_stats, on="day_of_week", how="left").fillna(
                0
            )
            ...
            fig_d.update_layout(
                title=dict(
                    text="By weekday", font=dict(size=12, color="#64748b"), x=0.01
                ),
                height=300,
                margin=dict(l=20, r=20, t=40, b=20),
                paper_bgcolor="rgba(0,0,0,0)",
        ...
        if not hw_df.empty:
            grid = (
                hw_df.groupby(["day_of_week", "hour_of_day"])
                .agg(pnl=("profit_usdt", "sum"))
                .reset_index()
            )
            # Volle 7x24 Matrix
            mat = np.zeros((7, 24))
        ...
        with rcol_l:
            fig_r = go.Figure(
                go.Bar(
                    x=reason_stats["count"],
                    y=reason_stats["reason"],
                    orientation="h",
                    marker=dict(color="#6a5cc0"),
                    text=reason_stats["count"],
                    textposition="outside",
            ...
            fig_r.update_layout(
                title=dict(
                    text="Frequency", font=dict(size=12, color="#64748b"), x=0.01
                ),
                height=320,
                margin=dict(l=20, r=20, t=40, b=20),
                paper_bgcolor="rgba(0,0,0,0)",
        ...
        with rcol_r:
            ...
            fig_rp.update_layout(
                title=dict(
                    text="P&L per reason", font=dict(size=12, color="#64748b"), x=0.01
                ),
                height=320,
                margin=dict(l=20, r=20, t=40, b=20),
                paper_bgcolor="rgba(0,0,0,0)",
...
with tab_bots:
    ...
    comp_bot_col = (
        "base_bot"
        if not bot_scope.empty and "base_bot" in bot_scope.columns
        else "bot_name"
    )
    for bot in ALL_BOTS:
        bdf = (
            bot_scope[bot_scope[comp_bot_col] == bot]
        ...
        comp_rows.append(
            {
                ...
                if bm.get("profit_factor") != float("inf")
                else 999,
    ...
    comp_df_fmt["Total P&L"] = comp_df_fmt["Total P&L"].apply(
        lambda v: f"{v:+.2f} USDT"
    )
    ...
    if not params_df.empty:
        ...
        for idx, bot in enumerate(ALL_BOTS):
            with param_cols[idx]:
                ...
                if bot_params.empty:
                    rows_html = (
                        '<div style="color:#64748b; padding:8px; font-size:0.8rem;">'
    ...
    if learning_df.empty:
        ...
        _render_dark_table(
            display_l, max_rows=200, height_px=380, align_right_cols=["Old", "New"]
        )
