...
def futures_unrealized_from_row(row: Any) -> tuple[float, float]:
    ...
    try:
        ...
        if stored_pnl != 0.0 and stored_pct != 0.0:
            if (stored_pnl > 0.0) == (stored_pct > 0.0):
        ...
        side = str(get("position_type", "") or "").strip().upper()
        ...
        pnl = stored_pnl if stored_pnl != 0.0 else calc_pnl
        pct = stored_pct if stored_pct != 0.0 else calc_pct
        if pnl != 0.0 and pct != 0.0 and (pnl > 0.0) != (pct > 0.0):
            return calc_pnl, calc_pct
        return pnl, pct
    except Exception:
        return 0.0, 0.0
...
def is_futures_state_fresh(
    ...
    *,
    ...
    """
    max_age = _finite_float(max_age_sec, math.nan)
...
def state_file_age_sec(path: str, *, now_ts: float | None = None) -> float | None:
    try:
        mtime = _finite_float(os.path.getmtime(path), math.nan)
    except (OSError, TypeError, ValueError, OverflowError):
        return None
    ref = (
        datetime.now(timezone.utc).timestamp()
        if now_ts is None
        else _finite_float(now_ts, math.nan)
    )
    if not math.isfinite(mtime) or not math.isfinite(ref):
        return None
    age = ref - mtime
