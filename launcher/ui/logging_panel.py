...
_DISPLAY_ACTIVITY_SUMMARY_RE = re.compile(
    r"^\s*INFO\s+\[Activity\]\s+\d+\s+routine\s+messages?\s+"
    r"condensed\s+over\s+\d+s\s+\(.+\)\.\s+"
    r"Enable Details for raw output\.\s*$",
    re.IGNORECASE,
)
...
class LiveLogDisplayFilter:
    ...
    @staticmethod
    def _warning_scope(fingerprint: str) -> str | None:
        match = re.match(r"^\[([^\]\r\n]{1,80})\]\s+", fingerprint)
        return match.group(1) if match is not None else None

    def _rearm_warning_scope(self, line: str) -> None:
        fingerprint = self._fingerprint(line)
        scope = self._warning_scope(fingerprint)
        if scope is None:
            return
        self._warning_seen_at = {
            key: seen
            for key, seen in self._warning_seen_at.items()
            if self._warning_scope(key) != scope
        }
...
def classify_severity(line: str) -> str:
    ...
    if explicit == "INFO" and _DISPLAY_ACTIVITY_SUMMARY_RE.fullmatch(text):
        # This exact line is generated by LiveLogDisplayFilter itself.  Words
...
def begin_log_session(card: dict, bot_name: str, *, emit=None) -> None:
    """Close display-only aggregation and mark a new visible bot session."""
    if emit is None:
        emit = log_to_card
    ...
    try:
        ...
        pass
    emit(card, "system", f"--- New {bot_name} session ---")
