...
class Sparkline(tk.Frame):
    ...
    def __init__(self, parent, width=180, height=34):
        super().__init__(
            parent, bg=COLORS["panel_alt"], highlightthickness=0, bd=0, height=height
        )
        ...
        self.canvas = tk.Canvas(
            self, height=height, bg=COLORS["panel_alt"], highlightthickness=0, bd=0
        )
    ...
    def _redraw(self) -> None:
        ...
        if not vals or len(vals) < 2 or w < 4 or h < 4:
            ...
            c.create_line(2, h / 2, w - 2, h / 2, fill=COLORS["border"], width=1)
            return
        ...
        if vmin < 0 < vmax:
            zy = _y(0.0)
            c.create_line(
                pad, zy, w - pad, zy, fill=COLORS["border"], width=1, dash=(2, 3)
            )
    ...
    * Calls ``on_change(key, value)`` on any change
    ...
    def _flash(self):
        ...
        self.after(
            400,
            lambda: (
                self.value_lbl.configure(text_color=COLORS["text"])
                if self.value_lbl.winfo_exists()
                else None
            ),
        )
...
class BadHoursRow(ctk.CTkFrame):
    ...
    def __init__(self, parent, bot_name: str, accent: str, mono_font: str):
        ...
        attach_tooltip(
            self._entry,
            "Press Enter or click away to save  (values in local time)",
            delay_ms=600,
        )
